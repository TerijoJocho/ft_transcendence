    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-8 flex flex-col items-center gap-6 shadow-2xl max-w-sm w-full mx-4">
        ...
        <div className="flex gap-4 w-full">
          <button onClick={onConfirm} className="flex-1 py-3 text-sm uppercase tracking-widest border border-rose-700 text-rose-400 rounded-md hover:bg-rose-700/20 hover:border-rose-500 transition-all">Confirmer</button>
          <button onClick={onCancel} className="flex-1 py-3 text-sm uppercase tracking-widest border border-gray-700 text-gray-400 rounded-md hover:bg-gray-700/20 hover:border-gray-500 transition-all">Annuler</button>
        </div>
      </div>
    </div>
  );
...
function ChessGame({
  ...
  const isOnline = online.enabled && online.gameId !== null && online.playerColor !== null;
  ...
  const persistEndOfGame = useCallback(
    async (result: { winner: string; reason: string }, moveCount: number) => {
      ...
      try {
        if (isDraw) {
          await endGame(online.gameId, { totalNbMoves: moveCount, winnerNbMoves, gameResult: "DRAW" });
        } else {
          const winnerColor = result.winner === "White" ? "WHITE" : "BLACK";
          await endGame(online.gameId, { totalNbMoves: moveCount, winnerNbMoves, gameResult: "WIN", winnerColor });
        }
      } catch { /* best effort */ }
    },
  ...
  const exportSnapshot = useCallback(() => {
    ...
    return { gameId: online.gameId, board, turn: player, moved, ep, halfmove, history, gameOver, status, lastMove, whiteTime, blackTime, clockStarted, gameResult };
  }, [online.gameId, board, player, moved, ep, halfmove, history, gameOver, status, lastMove, whiteTime, blackTime, clockStarted, gameResult]);

  const applySnapshot = useCallback((snapshot: GameStateSnapshot | null) => {
    if (!snapshot) return;
    isApplyingRemoteRef.current = true;
    setBoard(snapshot.board); setPlayer(snapshot.turn); setMoved(snapshot.moved);
    setEp(snapshot.ep); setHalfmove(snapshot.halfmove); setHistory(snapshot.history ?? []);
    setGameOver(Boolean(snapshot.gameOver)); setStatus(snapshot.status ?? "");
    setLastMove(snapshot.lastMove ?? null);
    setWhiteTime(snapshot.whiteTime ?? initSeconds); setBlackTime(snapshot.blackTime ?? initSeconds);
    setClockStarted(Boolean(snapshot.clockStarted)); setGameResult(snapshot.gameResult ?? null);
    setPendingPromo(null); setSelected(null); setHints([]);
  ...
  useEffect(() => {
    if (gameOver) {
      redirectTimerRef.current = setTimeout(() => { onBack(); }, 5000);
    }
  ...
  function handleDrop(e: React.DragEvent, dr: number, dc: number) {
    ...
    const [sr, sc] = dragSource; setDragSource(null);
  ...
  async function handleGiveUpConfirm() {
    ...
    const winnerNbMoves = player === "white" ? Math.floor(totalNbMoves / 2) : Math.ceil(totalNbMoves / 2);
    if (isOnline && online.gameId && !gameOver) {
      try {
        await giveupGame(online.gameId, { totalNbMoves, winnerNbMoves });
        if (socketRef.current?.connected) socketRef.current.emit("giveup", { gameId: online.gameId });
  ...
  async function handleCancelConfirm() {
    ...
    if (isOnline && online.gameId && !gameOver) {
      try {
        ...
        if (socketRef.current?.connected) socketRef.current.emit("cancel", { gameId: online.gameId });
  ...
  function handleQuit() {
    ...
    if (resolvedGameStatus === "PENDING") { setConfirmAction("cancel"); }
    else if (resolvedGameStatus === "ONGOING") { setConfirmAction("giveup"); }
    else { onBack(); }
  }
    ...
    <div className="text-white w-full bg-white">
      ...
      <div className="flex gap-6 p-4 sm:p-6 justify-center items-start flex-wrap">
        ...
        <div className="w-52 bg-gray-900 border border-gray-700 rounded-lg p-4 flex flex-col" style={{ height: "520px" }}>
          <p className="text-xs uppercase tracking-widest text-gray-500 mb-2 pb-2 border-b border-gray-700 flex-shrink-0">Moves</p>
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {hasClock && (
            <div className="flex items-center gap-3 self-end">
              <span className="text-xs uppercase tracking-widest text-gray-500">{topClockLabel}</span>
              <Clock seconds={topClockSeconds} active={player === topClockPlayer && clockStarted && !gameOver} low={topClockSeconds <= lowThreshold} />
            </div>
          )}
          {!hasClock && (
          ...
          {isOnline && onlineStatus && onlineStatus.includes("attente") && (
            <p style={{ color: "#7c3aed", fontWeight: 600 }} className="text-xs tracking-wide text-center uppercase">
              {onlineStatus}
            </p>
          )}
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <div className="flex justify-around pt-1 text-xs text-gray-600 select-none">
                  {fileLabels.map(l => <span key={l}>{l}</span>)}
                </div>
              </div>
            </div>
          </div>
          ...
          {hasClock && (
            <div className="flex items-center gap-3 self-end">
              <span className="text-xs uppercase tracking-widest text-gray-500">{botClockLabel}</span>
              <Clock seconds={botClockSeconds} active={player === botClockPlayer && clockStarted && !gameOver} low={botClockSeconds <= lowThreshold} />
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-40">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 flex flex-col gap-4 shadow-2xl w-full max-w-sm mx-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-sm font-semibold ${gameResult.winner === "Draw" ? "text-gray-300" : "text-rose-400"}`}>
                {gameResult.winner === "Draw"
                  ? `Draw — ${gameResult.reason}`
                  : gameResult.reason === "resign" || gameResult.winner === "opponent"
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="flex flex-col items-center bg-gray-800 rounded-lg py-3">
                ...
                <span className="text-2xl font-semibold text-white">{history.length}</span>
              </div>
              <div className="flex flex-col items-center bg-gray-800 rounded-lg py-3">
                <span className="text-xs text-gray-500 uppercase tracking-widest mb-1">White</span>
                <span className="text-2xl font-semibold text-white">{Math.ceil(history.length / 2)}</span>
              </div>
              <div className="flex flex-col items-center bg-gray-800 rounded-lg py-3">
                <span className="text-xs text-gray-500 uppercase tracking-widest mb-1">Black</span>
                <span className="text-2xl font-semibold text-white">{Math.floor(history.length / 2)}</span>
              </div>
            </div>
            <p className="text-xs text-gray-500 text-center">Retour automatique dans 5s…</p>
            <button
              onClick={() => {
...
function Game() {
  ...
  const [activeGameStatus, setActiveGameStatus] = useState<"PENDING" | "ONGOING" | "COMPLETED" | null>(null);
  ...
  const loadPendingGames = useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Impossible de charger les parties en attente.";
          ...
          <div className="flex flex-col gap-6 items-center w-full max-w-3xl px-6">
            ...
            <div className="w-full bg-white rounded-xl border border-violet-400 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => void loadPendingGames()} className="px-3 py-2 text-sm border border-violet-400 text-violet-500 rounded-md hover:bg-violet-50 transition">
                  Rafraîchir
                </button>
              </div>
          ...
          <div className="flex flex-col items-center gap-6">
            <div className="flex flex-row gap-4">
              {(["Local", "En ligne"] as Mode[]).map((m) => (
                <button key={m} onClick={() => { setMode(m); if (m === "Local") setColor("Blanc"); }} className={`${btnBase} ${mode === m ? btnActive : btnInactive}`}>{m}</button>
              ))}
            </div>
            <div className="flex flex-row gap-4">
            ...
            {menuError && <p className="text-sm text-red-500">{menuError}</p>}
            <button
              onClick={() => { setShowCreateOptions(false); setMode("Local"); setTimeControl("Normal"); setColor("Blanc"); setMenuError(null); }}

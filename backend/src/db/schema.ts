...
export const gameTable = pgTable("games", {
  ...
  (pgTable) => [
    check("checkmate", sql`${pgTable.gameStatus} <> 'COMPLETED' OR (${pgTable.totalNbMoves} IS NOT NULL AND ${pgTable.winnerNbMoves} IS NOT NULL)`),
  ]
);
...
export const participationTable = pgTable("participation", {
  ...
  (pgTable) => [
    foreignKey({ columns: [pgTable.playerId], foreignColumns: [playerTable.playerId] }).onDelete('restrict').onUpdate('cascade'),
    foreignKey({ columns: [pgTable.gameId], foreignColumns: [gameTable.gameId] }).onDelete('cascade').onUpdate('cascade'),
    unique().on(pgTable.playerId, pgTable.gameId),
    unique().on(pgTable.gameId, pgTable.playerColor),
    uniqueIndex("one_current_game_only").on(pgTable.playerId).where(sql`${pgTable.playerResult} = 'PENDING'`),
  ]
);
...
export const friendshipTable = pgTable("friendship", {
  ...
  (pgTable) => [
    ...
    foreignKey({ columns: [pgTable.player2Id], foreignColumns: [playerTable.playerId] }).onDelete('restrict').onUpdate('cascade'),
    unique().on(pgTable.player1Id, pgTable.player2Id),
    check("no_self_friendship", sql`${pgTable.player1Id} <> ${pgTable.player2Id}`),
    check("ordered_player_ids", sql`${pgTable.player1Id} < ${pgTable.player2Id}`)
  ]
);

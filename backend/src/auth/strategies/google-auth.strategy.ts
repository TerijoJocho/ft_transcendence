...
export class GoogleAuthStrategy extends PassportStrategy(Strategy, 'google') {
  ...
  async validate(_accessToken: string, _refreshToken: string, profile: any) {
	...
	try {
		const isExist = (await this.utilsService.findPlayersBy("and", undefined, eq(playerTable.mailAddress, emails[0].value))) as playerSelect[];
		if (isExist.length === 0) {
			const user = (await this.signinService.registerPlayers(
				emails[0].value,
				displayName
			))[0];
			return user;
